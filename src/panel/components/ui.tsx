...
/** Executor glyphs — crisp SVGs instead of platform-dependent emoji. */
export function CloudIcon(props: { size?: number }) {
  const size = props.size ?? 11;
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden class="shrink-0">
      <path d="M4.5 12.5a3 3 0 0 1-.4-5.97 4 4 0 0 1 7.8-.9A3.2 3.2 0 0 1 11.5 12.5h-7Z" />
    </svg>
  );
}

export function MonitorIcon(props: { size?: number }) {
  const size = props.size ?? 11;
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden class="shrink-0">
